...
module PfinAsFreeJoinSemilattice where
...
x ∈ₛ ass s₁ s₂ s₃ i = 
  ⇔toPath {_} {x ∈ₛ s₁ ⊔ x ∈ₛ s₂ ⊔ x ∈ₛ s₃} {(x ∈ₛ s₁ ⊔ x ∈ₛ s₂) ⊔ x ∈ₛ s₃}
    (∥rec∥ isPropPropTrunc λ { (inj₁ m) → inl (inl m)
                            ; (inj₂ m) → ∥map∥ (map⊎ inr (λ y → y)) m})
    (∥rec∥ isPropPropTrunc λ { (inj₁ m) → ∥map∥ (map⊎ (λ y → y) inl) m
                            ; (inj₂ m) → inr (inr m)})
    i
x ∈ₛ idem s i =
...
isPropPfinRel R s t =
  isProp× (isPropΠ (λ _ → isPropΠ (λ _ → isPropPropTrunc)))
          (isPropΠ (λ _ → isPropΠ (λ _ → isPropPropTrunc)))  
...
pre∈ₛmapPfin f b =
  elimPfinProp (λ x → _ , isPropΠ (λ _ → isPropPropTrunc))
    (λ ())
    (λ a → ∥map∥ (λ eq → a , ∣ refl ∣ , sym eq))
    λ p₁ p₂ → ∥rec∥ isPropPropTrunc
...
⊆∪ s1 s2 =
  elimPfinProp (λ _ → _ , isPropΠ (λ _ → isPropPropTrunc))
