...
--*************************************--
--*************************************--
-- Combining the monads Lift and State --
--*************************************--
--*************************************--
...
-- Then I tried formulating a distributive law between the State monad and the Lift monad, but that did not
-- quite result in the combined monad I had in mind.
-- So Instead I simply define the monad I had in mind, prove that it is a monad
-- and show its equational theory, which has interaction between time steps and lookup/update functions from state.
...
bindS : {A B S : Set} → (A → State S B) → State S A → State S B
bindS h f = λ s → h (proj₂ (f s)) (proj₁ (f s))
